...
import { GalleryItemDoc, ScheduleDoc, UserDoc } from '@/types'
import {
	addDoc,
	collection,
	deleteDoc,
...
export async function uploadSchedule(document: ScheduleDoc) {
	const ref = collection(dbService, 'schedule')

	const newScheduleDoc = await addDoc(ref, document)
	const newScheduleRef = doc(dbService, 'schedule', newScheduleDoc.id)
...
export async function getAllSchedule() {
	const collectionRef = collection(dbService, 'schedule')
	...
	try {
		...
		return documents as ScheduleDoc[]
	} catch (error) {
...
export async function deleteGalleryItem(imageDoc: GalleryItemDoc) {
	...
	if (imageDoc.isGif) {
		...
		imageRef = ref(storageService, `gallery/images/${imageDoc.storageFileName}`)
	}
